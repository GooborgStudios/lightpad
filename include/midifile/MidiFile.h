...
class MidiFile {
	public:
		...
		int addEvent(int aTrack, int aTick, vector<uchar> &midiData);
		int addEvent(MidiEvent &mfevent);

		// MIDI message adding convenience functions:
		int addNoteOn(int aTrack, int aTick, int aChannel, int key, int vel);
		int addNoteOff(int aTrack, int aTick, int aChannel, int key, int vel);
		int addNoteOff(int aTrack, int aTick, int aChannel, int key);
		int addController(int aTrack, int aTick, int aChannel, int num, int value);
		int addPatchChange(int aTrack, int aTick, int aChannel, int patchnum);
		int addTimbre(int aTrack, int aTick, int aChannel, int patchnum);
		int addPitchBend(int aTrack, int aTick, int aChannel, double amount);

		// Meta-event adding convenience functions:
		int addMetaEvent(int aTrack, int aTick, int aType, vector<uchar> &metaData);
		int addMetaEvent(int aTrack, int aTick, int aType, const char *metaData);
		int addCopyright(int aTrack, int aTick, const string &text);
		int addTrackName(int aTrack, int aTick, const string &name);
		int addInstrumentName(int aTrack, int aTick, const string &name);
		int addLyric(int aTrack, int aTick, const string &text);
		int addMarker(int aTrack, int aTick, const string &text);
		int addCue(int aTrack, int aTick, const string &text);
		int addTempo(int aTrack, int aTick, double aTempo);
		int addTimeSignature(int aTrack, int aTick, int top, int bottom, int clocksPerClick = 24,
		                     int num32dsPerQuarter = 8);
		int addCompoundTimeSignature(int aTrack, int aTick, int top, int bottom, int clocksPerClick = 36,
		                             int num32dsPerQuarter = 8);
	...
	private:
		int extractMidiData(istream &inputfile, vector<uchar> &array, uchar &runningCommand);
