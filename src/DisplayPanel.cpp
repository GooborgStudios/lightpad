...
DisplayPanel::DisplayPanel(wxPanel *parent)
	...
	image_ypos = 0;
		...
		for (int i = 0; i < 6; i++)
			launchpad_button_images[i] = new wxImage(launchpad_button_image->Size(wxSize(286, 286), wxPoint(-286*i, 0)));
	#else
	...
	for (int j = 0; j < 18; j++) {
		...
		rainbow[j] = wxColor(val_in_range(red,0,255), val_in_range(grn,0,255), val_in_range(blu,0,255));
	}
	for (int i = 1; i < 99; i++) {
		...
		if (x == 0 || x == 9 || y == 0 || y == 9) {
			...
			for (int bmp_x = 0; bmp_x < width; bmp_x++) {
				for (int bmp_y = 0; bmp_y < width; bmp_y++) {
					...
					float r_percent = pixel.Red()/255.0, g_percent = pixel.Green()/255.0, b_percent = pixel.Blue()/255.0;
